                            ...
                            @else
                                @foreach ($leads as $i => $lead)
                                    ...
                                    <tr>
                                        ...
                                        {{-- @php
                                            $qs = json_decode($lead->prediction, true);
                                        @endphp
                                        <td>
                                            @if (!empty($qs['status']))
                                                <span
                                                    class="badge badge-{{ $qs['status'] === 'real' ? 'success' : 'danger' }}">
                                                    {!! $qs['status'] === 'real' ? '🟢 Real' : '🔴 Fake' !!}
                                                </span>
                                            @endif
                                            @if (!empty($qs['strength']))
                                                <span
                                                    class="badge badge-{{ $qs['strength'] === 'hot' ? 'danger' : ($qs['strength'] === 'warm' ? 'warning' : 'secondary') }}">
                                                    {!! $qs['strength'] === 'hot' ? '🔥 Hot' : ($qs['strength'] === 'warm' ? '🌤️ Warm' : '❄️ Cold') !!}
                                                </span>
                                            @endif
                                            ...
                                            @endif
                                            @if (!empty($qs['sentiment']))
                                                <span
                                                    class="badge badge-{{ $qs['sentiment'] === 'positive' ? 'success' : ($qs['sentiment'] === 'negative' ? 'danger' : 'secondary') }}">
                                                    {!! $qs['sentiment'] === 'positive'
                                                        ? '😊 Positive'
                                                        : ($qs['sentiment'] === 'negative'
                                                            ? '😡 Negative'
                                                            : '😐 Neutral') !!}
                                                </span>
                                            @endif
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('seller.leads.delete') }}" class="leadform" id="form1">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <div class="form-group mb-3">
                                    <select name="status" class="form-control">
                                        ...
                                        <option value="proposal_sent">Proposal
                                            Sent</option>
